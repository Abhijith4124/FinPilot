defmodule Finpilot.TasksTest do
  ...
  describe "task_stages" do
    ...
    test "list_task_stages/0 returns all task_stages" do
      ...
      assert Tasks.list_task_stages() == [task_stage]
    end

    test "get_task_stage!/1 returns the task_stage with given id" do
      task_stage = task_stage_fixture()
      assert Tasks.get_task_stage!(task_stage.id) == task_stage
    end
    ...
    test "create_task_stage/1 with valid data creates a task_stage" do
      ...
      assert {:ok, %TaskStage{} = task_stage} = Tasks.create_task_stage(valid_attrs)
    ...
    test "create_task_stage/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Tasks.create_task_stage(@invalid_attrs)
    end
    ...
    test "update_task_stage/2 with valid data updates the task_stage" do
      ...
      assert {:ok, %TaskStage{} = task_stage} = Tasks.update_task_stage(task_stage, update_attrs)
    ...
    test "update_task_stage/2 with invalid data returns error changeset" do
      task_stage = task_stage_fixture()
      assert {:error, %Ecto.Changeset{}} = Tasks.update_task_stage(task_stage, @invalid_attrs)
      assert task_stage == Tasks.get_task_stage!(task_stage.id)
    end

    test "delete_task_stage/1 deletes the task_stage" do
      task_stage = task_stage_fixture()
      assert {:ok, %TaskStage{}} = Tasks.delete_task_stage(task_stage)
      assert_raise Ecto.NoResultsError, fn -> Tasks.get_task_stage!(task_stage.id) end
    end

    test "change_task_stage/1 returns a task_stage changeset" do
      task_stage = task_stage_fixture()
      assert %Ecto.Changeset{} = Tasks.change_task_stage(task_stage)
    end
  end
